...
namespace WindowsFormsApplication
{
    class BUS_TTKM
    {
        ...
        {
            db = new CMART8Entities();
            //return db.KHUYENMAIs.Where(st => st.MAKM.Contains(sTmp) || st.MASP.Contains(sTmp) || st.NOIDUNG.Contains(sTmp) || st.HINHANH.Contains(sTmp) || st.GIAKM.Equals(float.Parse(sTmp)) || st.THOIGIANBD.Equals(DateTime.Parse(sTmp)) || st.THOIGIANKT.Equals(DateTime.Parse(sTmp))).ToList();
            return db.KHUYENMAIs.Where(st => st.MAKM.Contains(sTmp) || st.MASP.Contains(sTmp) || st.NOIDUNG.Contains(sTmp) || st.HINHANH.Contains(sTmp)).ToList();
        }
